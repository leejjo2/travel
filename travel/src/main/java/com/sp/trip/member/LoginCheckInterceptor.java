  ...
  * 인터셉터를 적용하기 위해서는 아래와 같이 HandlerInterceptor를 등록 해야 한다.
    ...
    <mvc:interceptors>
      <mvc:interceptor>
          ...
          <bean class="com.sp.trip.member.LoginCheckInterceptor"/>
      </mvc:interceptor>
    </mvc:interceptors>
...
public class LoginCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse resp, Object handler) throws Exception {
		...
		try {
			...
			if (info == null) {
				...
				if(uri.indexOf("admin")!=-1 && info.getMembership() < 51) { // membership이 50 이하인 사용자가 관리자 페이지에 접근하는 경우
					result = false;
					resp.sendRedirect(cp+"/member/noAuthorized"); // 권한 없다는 페이지로 리다이렉트
				}
	...
	/*
	 * 컨트롤러가 요청을 처리 한 후에 호출. 컨트롤러 실행 중 예외가 발생하면 실행하지 않음
	 */
	@Override
